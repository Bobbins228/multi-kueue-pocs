...
CURRENT_CONTEXT=$(kubectl --kubeconfig=worker-config config current-context)
CURRENT_CLUSTER=$(kubectl --kubeconfig=worker-config config view -o jsonpath="{.contexts[?(@.name == \"${CURRENT_CONTEXT}\"})].context.cluster}")
CURRENT_CLUSTER_ADDR=$(kubectl --kubeconfig=worker-config config view -o jsonpath="{.clusters[?(@.name == \"${CURRENT_CLUSTER}\"})].cluster.server}")
